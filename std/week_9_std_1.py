...
class Ship:
    ...
    def draw(self,canvas):
        if self.thrusters:
            canvas.draw_image(self.image, [ship_info.get_center()[0] + ship_info.get_size()[0],ship_info.get_center()[1]], ship_info.get_size(), self.pos, [3 * ship_info.get_radius()]*2, self.angle)
        else:
            canvas.draw_image(self.image, ship_info.get_center(), ship_info.get_size(), self.pos, [3 * ship_info.get_radius()]*2, self.angle)
    ...
    def shoot(self):
        ...
        a_missile = Sprite([self.pos[0] + 50*angle_to_vector(self.angle)[0], self.pos[1] + 50*angle_to_vector(self.angle)[1]],
...
class Sprite:
    ...
    def draw(self, canvas):
        ...
        canvas.draw_image(self.image, self.image_center, self.image_size, self.pos, [2 * self.radius]*2, self.angle)
...
def draw(canvas):
    ...
    canvas.draw_image(debris_image, center, size, (wtime + WIDTH / 2, HEIGHT / 2), (WIDTH / 10, HEIGHT / 10))
    ...
    canvas.draw_text("Score: %d" % score, [625, 40], 40, "Magenta", "sans-serif")
...
def rock_spawner():
    ...
    if len(rock_group) < 12:
        a_rock = Sprite([random.randrange(WIDTH), random.randrange(HEIGHT)],
                        [(1 + 0.01 * rock_modifier) * random.randrange(1,100)/100.0, (1 + 0.01 * rock_modifier) * random.randrange(1,100)/100.0],
                        random.randrange(365)/12.56,
                        random.randrange(100)/1000.0, asteroid_image, asteroid_info)
