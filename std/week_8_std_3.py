...
ship_thrust_sound.set_volume(0.6)
...
class Ship:
    ...
    def draw(self,canvas):
        center_source = list(self.image_center)
        if self.thrust:
            center_source[0] += self.image_size[0]
        canvas.draw_image(self.image, center_source, self.image_size, self.pos, self.image_size, self.angle)
    ...
    def rotate(self, left_right):
        self.angle_vel = left_right
        
        
    def shoot(self):
        global a_missile
        pos = angle_to_vector(self.angle)
        startpos = [self.pos[0] +pos[0]*self.radius, self.pos[1] +pos[1]*self.radius]
        ...
        a_missile = Sprite(startpos , vel, 0, 0, missile_image, missile_info, missile_sound)
        missile_sound.play()
...
class Sprite:
    def __init__(self, pos, vel, ang, ang_vel, image, info, sound = None):
        ...
        self.age = 0
        self.rotate_speed = random.random()/10
...
def draw(canvas):
    ...
    canvas.draw_text('Lives:', (40, 40), 32, 'white')
    canvas.draw_text(str(lives), (40, 80), 32, 'white')
    canvas.draw_text('Score:', (700, 40), 32, 'white')
    canvas.draw_text(str(score), (700, 80), 32, 'white')
...
def rock_spawner():
    global a_rock
    startpos = [random.randrange(WIDTH), random.randrange(HEIGHT)]	#[WIDTH / 3, HEIGHT / 3]

    vel = [random.randrange(-50, 50)/10, random.randrange(-50, 50)/10]
    a_rock = Sprite(startpos, vel, 0, 0, asteroid_image, asteroid_info)
