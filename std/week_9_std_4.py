...
def draw(canvas):
    ...
    # process ship_collsion
    if(group_collide(rock_group,my_ship)):
        lives = lives - 1
    
    # process missile_and_rock
    ...
    # process rock_group and missile_group
...
def group_group_collide(group1,group2):
    remove_elements1 = set([])
    remove_elements2 = set([])
    for sprite1 in group1:
        for sprite2 in group2:
            if sprite1.collide(sprite2):
                remove_elements1.add(sprite1)
                remove_elements2.add(sprite2)
                explosion_group.add(Sprite(sprite1.pos, [0, 0], 0, 0, explosion_image, explosion_info))
                explosion_sound.rewind()
                explosion_sound.play()
    
    if len(remove_elements1) > 0:
        group1.difference_update(remove_elements1)
    if len(remove_elements2) > 0:
        group2.difference_update(remove_elements2)
    
    return len(remove_elements1)>0 
...
def group_collide(group,other_object):
    ...
    for sprite in group:
        if sprite.collide(other_object):
            remove_elements.add(sprite)
            collision_no = collision_no + 1
            explosion_group.add(Sprite(sprite.pos, [0, 0], 0, 0, explosion_image, explosion_info))
            explosion_sound.rewind()
            explosion_sound.play()
            
    if len(remove_elements)>0:
        group.difference_update(remove_elements)
      
    return collision_no>0
