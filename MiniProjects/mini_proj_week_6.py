...
#http://www.codeskulptor.org/#user40_JR8AH09yW6_16.py
import simplegui
import random

# Load images
image_card_back = simplegui.\
  load_image("https://dl.dropboxusercontent.com/u/40444366/card_back.png?dl=1")
image_cards = simplegui.\
  load_image("https://dl.dropboxusercontent.com/u/40444366/cards.png?dl=1")
...
def mouseclick(pos):
    ...
    for r in range(4):
        for c in range(4):
            if CARD_WIDTH * c < pos[0] < CARD_WIDTH * (c + 1) and \
                 CARD_HEIGHT * r < pos[1] < CARD_HEIGHT * (r + 1):
                if face_down[i]:
                    face_down[i] = False
                    if state == 0:
                        first_index = i
        ...
        for c in range(4):
            if face_down[r * 4 + c]:
                canvas.draw_image(image_card_back,
                                  ...
                                  [CARD_WIDTH * (c + 1) - CARD_WIDTH // 2 - 1, \
                                   CARD_HEIGHT * (r + 1) - CARD_HEIGHT // 2 - 1],
                                  [CARD_WIDTH, CARD_HEIGHT])
            else:
                canvas.draw_image(image_cards,
                                  [cards[r * 4 + c][0], cards[r * 4 + c][1]], \
                                  [CARD_WIDTH, CARD_HEIGHT],
                                  [CARD_WIDTH * (c + 1) - CARD_WIDTH // 2 - 1, \
                                   CARD_HEIGHT * (r + 1) - CARD_HEIGHT // 2 - 1],
                                  [CARD_WIDTH, CARD_HEIGHT])
